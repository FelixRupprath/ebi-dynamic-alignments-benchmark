        ...
        if current_state.leaves:
            if current_state.index == len(variant):
                ...
                if variant[-1] in leaf_labels:
                    ...
                    if (
                        current_state.costs - sum(
                            leaf.label is not None for leaf in current_state.leaves
                        )
                        > parent_state.costs
                    ):
                        alignment.append(
...
def _check_if_state_exists_and_update(
    search_state: SGASearchState, collection, open_set
):
    ...
    for alt in collection:
        if alt.index == search_state.index and alt.state == search_state.state:
            ...
            if search_state.costs < alt.costs:
                ...
                costs_updated = True
    if costs_updated:
        # Updating an existing state (or its descendants) changes heap priorities.
        # Repair only after a decrease, including decreases found in closed states.
        heapq.heapify(open_set)
    return match


def _add_new_state(state, parent, open, closed):
    if not _check_if_state_exists_and_update(state, closed, open):
        if not _check_if_state_exists_and_update(state, open, open):
